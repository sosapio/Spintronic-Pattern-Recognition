...
class LLG:
    ...
    def __init__(self,
                 ...
                 d_theta_fake_th=0.01,          # d_theta will have an extra
                                                # term based on this
                 theta_windowing=def_window,    # theta windowing function
                                                # should min theta be forced
                 theta_windowing_factor=3,      # theta_min = factor*theta_0
                 seed=0,
                 ...
                 alpha=0.01,                   # alpha damping factor
                 ms=1.2e6,                     # magnetization sat. [A/m]
                 # shape anisotropy
                 ...
                 lambda_s=1.0,                  # [FITTING] parameter
                 p=0.75,                        # polarization factor
                 # stt mode simplest
    ...
    def get_r(self, m_cart):
        ...
        https://doi.org/10.1109/TMAG.2017.2788010
        """
        return self.r_p * (1 + self.tmr * (1 - m_cart[2]) / 2)
    
    #self.r_p*(1 + self.tmr/(self.tmr + 2))/(
    #        1 - self.tmr/(self.tmr + 2)*m_cart[2])
        
    
    def compute_conductance(self):
        """Compute the maximum and minimum conductance."""
        
        # Parallel state (m_cart[2] = 1)
        m_cart_parallel = [0, 0, 1]  # z-component = 1
        resistance_parallel = self.get_r(m_cart_parallel)
        conductance_parallel = 1 / resistance_parallel  # Conductance is the inverse of resistance

        # Antiparallel state (m_cart[2] = -1)
        m_cart_antiparallel = [0, 0, -1]  # z-component = -1
        resistance_antiparallel = self.get_r(m_cart_antiparallel)
        conductance_antiparallel = 1 / resistance_antiparallel
        
        # Parallel state (m_z = 1)
        r_parallel = self.get_r([0, 0, 1])  # Expected: R_P

        # Antiparallel state (m_z = -1)
        r_antiparallel = self.get_r([0, 0, -1])  # Expected: R_AP 

        # Intermediate state (m_z = 0)
        r_intermediate = self.get_r([0, 0, 0])  # Expected: Average of R_P and R_AP

        print("Resistance of Parallel State", resistance_parallel)
        print("Resistance of Anti-Parallel State", resistance_antiparallel)
        print(f"Maximum Conductance (Parallel state): {conductance_parallel:.6e} S")
        print(f"Minimum Conductance (Antiparallel state): {conductance_antiparallel:.6e} S")
    ...
    def state_plotter(self,
        ...
        if plot_simple:
            ...
            ax_2d.legend()
            ax_2d.set_ylim([-1, 1])
            state_ranges = [1.0, 0.75, 0.5, 0.25, 0.0, -0.25, -0.5, -0.75, -1.0]
            for y in state_ranges:
                ax_2d.axhline(y, color='black', linestyle='-', linewidth=1)
            ax_2d.grid()
            plt.title(title)
            plt.show()
            return
    ...
    def compute_voltage(self, times, states, current_function):
        """
        Compute the voltage across the MTJ over time.

        Args:
            times (array): Time points of the simulation.
            states (array): Magnetization states over time.
            current_function (callable): Function to compute current as a function of time.

        Returns:
            array: Voltage values over time.
        """
        # Convert states to Cartesian coordinates and extract z-component
        xyz = cart_from_spherical_fn(states.T)
        m_z = xyz[:, 2]

        # Compute resistance for each time point based on m_z
        resistances = np.array([self.get_r([0, 0, m_z_t]) for m_z_t in m_z])

        # Compute current at each time point
        currents = np.array([current_function(t) for t in times])
    ...
    def plot_voltage(self, times, voltages):
        """
        Plot voltage across the MTJ over time with.

        Args:
            times (array): Time points of the simulation.
            voltages (array): Voltage values over time.
        """
        # Set up the plot with a dark background
        plt.figure(figsize=(8, 6))

        # Plot the voltage
        plt.plot(times * 1e9, -voltages * 1e3, color='red', label="Voltage (e0)", linewidth=1.5)

        # Add axis labels and title
        plt.xlabel("Time (ns)", fontsize=12, color='black')
        plt.ylabel("Voltage (mV)", fontsize=12, color='black')
        plt.title("Transient Analysis", fontsize=14, color='black')
    ...
    def get_write_time(self, times, states):
        """
        Detect when the magnetization switches from one state to another.
        Write time is the time when m_z goes from 1 to -1 (or vice versa).
        """
        ...
        # Find the first point where m_z crosses zero
        for i in range(1, len(m_z)):
            if m_z[i-1] > 0 and m_z[i] < 0:  # switching from + to -
                return times[i]
            elif m_z[i-1] < 0 and m_z[i] > 0:  # switching from - to +
                return times[i]
    ...
    def plot_resistance(self, times, states):
        """
        ...
        # Plot resistance
        plt.plot(times * 1e9, resistances, color='blue', label="Resistance", linewidth=1.5)

        # Add axis labels and title
        plt.xlabel("Time (ns)", fontsize=12, color='black')
        plt.ylabel("Resistance (Ohms)", fontsize=12, color='black')
        plt.title("Resistance Over Time", fontsize=14, color='black')
    ...
    def plot_conductance_over_time_8_states(self, times, states):
        """
        Plot conductance values over time by mapping z-component of magnetization to one of 8 discrete conductance states based on predefined ranges. 

        Args:
            llg_instance (LLG): Instance of the LLG class.
            times (array): Time points of the simulation.
            states (array): Magnetization states over time.
        """
       
        
        # Compute conductance values based on the z-component of magnetization
        # Define the conductance levels for each of the 8 states
        min_conductance = 1 / self.get_r([0, 0, -1])  # Conductance for antiparallel state
        max_conductance = 1 / self.get_r([0, 0, 1])   # Conductance for parallel state
        conductance_levels = np.linspace(max_conductance,min_conductance, 8)
        print("conductance_levels", conductance_levels),
        xyz = cart_from_spherical_fn(states.T)
        m_z_values = xyz[:, 2]  # Z-components

        conductance_values = []
        for m_z in m_z_values:
            # Determine conductance level based on the range each m_z falls into
            if 0.75 < m_z <= 1.0:
                conductance_values.append(conductance_levels[0])
            elif 0.5 < m_z <= 0.75:
                conductance_values.append(conductance_levels[1])
            elif 0.25 < m_z <= 0.5:
                conductance_values.append(conductance_levels[2])
            elif 0.0 < m_z <= 0.25:
                conductance_values.append(conductance_levels[3])
            elif -0.25 < m_z <= 0.0:
                conductance_values.append(conductance_levels[4])
            elif -0.5 < m_z <= -0.25:
                conductance_values.append(conductance_levels[5])
            elif -0.75 < m_z <= -0.5:
                conductance_values.append(conductance_levels[6])
            elif -1.0 < m_z <= -0.75:
                conductance_values.append(conductance_levels[7])
            
            else:
                # If m_z is outside the expected range, assign None
                conductance_values.append(None)
                
        # Set up the plot
        plt.figure(figsize=(10, 6))

        # Plot conductance over time
        plt.plot(times * 1e9, conductance_values, label="Conductance", color='purple', linewidth=1.5)
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Conductance (S)", fontsize=12)
        plt.title("Conductance Over Time with 8 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()

        # Identify unique conductance values and their counts
        unique_conductance = np.unique([c for c in conductance_values if c is not None])
        print("Unique Conductance Values:")
        for conductance in unique_conductance:
            print(f"Conductance: {conductance:.6e} S")

            
    
    def plot_conductance_over_time_4_states(self, times, states):
        """
        Plot conductance values over time by mapping z-component of magnetization to one of 4 discrete conductance states based on predefined ranges. 
        ...
        """
        # Compute conductance values based on the z-component of magnetization
        # Define the conductance levels for each of the 4 states
        min_conductance = 1 / self.get_r([0, 0, -1])  # Conductance for antiparallel state
        max_conductance = 1 / self.get_r([0, 0, 1])   # Conductance for parallel state
        conductance_levels = np.linspace(max_conductance,min_conductance, 4)
        print("conductance_levels", conductance_levels),
        xyz = cart_from_spherical_fn(states.T)
        m_z_values = xyz[:, 2]  # Z-components over time
        ...
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Conductance (S)", fontsize=12)
        plt.title("Conductance Over Time with 4 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()
    ...
    def plot_resistance_over_time_8_states(self, times, states):
        """
        Plot resistance values over time and provide time intervals for 8 unique resistance states.
        ...
        # Define the resistance levels for each of the 8 states
        min_resistance = self.get_r([0, 0, -1])  # Resistance for antiparallel state
        max_resistance = self.get_r([0, 0, 1])   # Resistance for parallel state
        resistance_levels = np.linspace(max_resistance, min_resistance, 8)

        # Convert states to Cartesian coordinates and extract the z-component
        xyz = cart_from_spherical_fn(states.T)
        m_z_values = xyz[:, 2]  # Z-components over time

        resistance_values = []
        for m_z in m_z_values:
            # Map m_z to resistance levels
            
            if 0.75 < m_z <= 1.0:
                resistance_values.append(resistance_levels[0])
            elif 0.5 < m_z <= 0.75:
                resistance_values.append(resistance_levels[1])
            elif 0.25 < m_z <= 0.5:
                resistance_values.append(resistance_levels[2])
            elif 0.0 < m_z <= 0.25:
                resistance_values.append(resistance_levels[3])
            elif -0.25 < m_z <= 0.0:
                resistance_values.append(resistance_levels[4])
            elif -0.5 < m_z <= -0.25:
                resistance_values.append(resistance_levels[5])
            elif -0.75 < m_z <= -0.5:
                resistance_values.append(resistance_levels[6])
            elif -1.0 < m_z <= -0.75:
                resistance_values.append(resistance_levels[7])
            

            else:
                resistance_values.append(None)

        # Find unique resistance values and corresponding time intervals
        unique_resistances = np.unique([r for r in resistance_values if r is not None])
            ...
            for i, r in enumerate(resistance_values):
                if r == resistance:
                    ...
                    if start_time is not None:
                        intervals.append((start_time, times[i]))
                        start_time = None

            if start_time is not None:
                intervals.append((start_time, times[-1]))

            resistance_intervals[resistance] = intervals

        # Set up the plot
        plt.figure(figsize=(10, 6))
        plt.plot(times * 1e9, resistance_values, label="Resistance", color='purple', linewidth=1.5)
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Resistance (Ohms)", fontsize=12)
        plt.title("Resistance Over Time with 8 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()
        
        # Plot the resistance values between 0 ns and 20 ns for evaluation
        time_range_start = 0e-9  # 0 ns
        time_range_end = 20e-9    # 20 ns

        filtered_times = [t for t in times if time_range_start <= t <= time_range_end]
        filtered_resistances = [resistance_values[i] for i, t in enumerate(times) if time_range_start <= t <= time_range_end]

        plt.figure(figsize=(10, 6))
        plt.plot([t * 1e9 for t in filtered_times], filtered_resistances, label="Resistance (0-20 ns)", color='blue', linewidth=1.5)
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Resistance (Ohms)", fontsize=12)
        plt.title("Resistance Over Time (0-20 ns) with 8 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()
    
    
        # Print the intervals for each unique resistance value
        print("Time intervals for unique resistance values:")
        for resistance, intervals in resistance_intervals.items():
            print(f"Resistance: {resistance:.6e} Ohms")
            for interval in intervals:
                print(f"  Interval: {interval[0]:.2e} s to {interval[1]:.2e} s")
    
    
    
    
    
    def plot_resistance_over_time_4_states(self, times, states):
        """
        Plot resistance values over time and provide time intervals for 4 unique resistance states.
        ...
        # Define the resistance levels for each of the 4 states
        min_resistance = self.get_r([0, 0, -1])  # Resistance for antiparallel state
        max_resistance = self.get_r([0, 0, 1])   # Resistance for parallel state
        resistance_levels = np.linspace(max_resistance, min_resistance, 4)
        ...
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Resistance (Ohms)", fontsize=12)
        plt.title("Resistance Over Time with 4 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()
        
        # Plot only the resistance values between 0 ns and 20 ns
        time_range_start = 0e-9  # 0 ns
        time_range_end = 20e-9    # 20 ns
        ...
        plt.xlabel("Time (ns)", fontsize=12)
        plt.ylabel("Resistance (Ohms)", fontsize=12)
        plt.title("Resistance Over Time (0-20 ns) with 4 States", fontsize=14)
        plt.grid()
        plt.legend(fontsize=10)
        plt.show()
    ...
    def solve_and_plot(self,
        ...
        # Measure the write time (time to switch magnetization)
        write_time = self.get_write_time(ode.t, ode.y)
        
        print("Device Write time: ", write_time)

        voltage = self.compute_voltage(ode.t, ode.y, self.i_amp_fn)
        self.plot_voltage(ode.t, voltage)
        
        # Compute and plot resistance
        self.plot_resistance(ode.t, ode.y)
        self.compute_conductance()
        
        # Calculate conductance for each time point based on the z-component
        self.plot_conductance_over_time_8_states(ode.t, ode.y)
        self.plot_conductance_over_time_4_states(ode.t, ode.y)

        self.plot_resistance_over_time_8_states(ode.t, ode.y)
        self.plot_resistance_over_time_4_states(ode.t, ode.y)
        ...
        # Zoomed-In Plot (0-25ns)
        zoomed_indices = ode.t <= 25e-9  # Filter for 0-25ns
        title_zoomed = 'Zoomed-in Timescale (0-25ns)'
        if plot:
            self.state_plotter(title=title_zoomed,
                               times=ode.t[zoomed_indices],
                               states=ode.y[:, zoomed_indices],
                               h_ext_cart=self.h_ext_cart,
                               i_amp=self.i_amp_fn,
                               theta_0=self.theta_0,
                               plot_simple=plot_simple)
