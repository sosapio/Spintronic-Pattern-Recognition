...
parser.add_argument('--epochs', type=int, default=100, help='number of epochs to train (default: 32)')
...
parser.add_argument('--wl_weight', type = int, default=5)
parser.add_argument('--wl_grad', type = int, default=5)
parser.add_argument('--wl_activate', type = int, default=8)
parser.add_argument('--wl_error', type = int, default=8)
parser.add_argument('--inference', default=0)
parser.add_argument('--onoffratio', default=1.3)
...
parser.add_argument('--max_level', default=2)
parser.add_argument('--d2dVari', default=0)
parser.add_argument('--c2cVari', default=0)
parser.add_argument('--r_on', default=3000)
parser.add_argument('--r_off', default=9000)
parser.add_argument('--readNoise', default=False)
parser.add_argument('--sigmaVoltageNoise', default=500*10**(-6))
parser.add_argument('--readVoltage', default=0.050)
parser.add_argument('--cellType', default='SRAM')
...
args.d2dVari = 0.003            # device-to-device variation
args.nonlinearityLTP = 1   # nonlinearity in LTP
args.nonlinearityLTD = 1   # nonlinearity in LTD (negative if LTP and LTD are asymmetric)
...
try:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if args.readNoise:
                min_weight_delta = 2/args.max_level #
                min_r_delta = (args.r_off - args.r_on)/args.max_level
                sigmaReadNoise = args.sigmaVoltageNoise/args.readVoltage
                
                if args.cellType == 'SRAM':
                    for name, param in list(model.named_parameters())[::-1]:
                        
                        r_states = ((param.data+1)/min_weight_delta)
                        noise = torch.normal(mean=0, std=torch.ones(*r_states.size(), args.wl_weight)*args.sigmaVoltageNoise)
                        idx = noise > 0.100
                        
                        if idx.sum() > 0:
                            print('The unthinkable has happened...')
                            sys.exit()
                
                else:
                    for name, param in list(model.named_parameters())[::-1]:

                        r_states = ((param.data+1)/min_weight_delta)*min_r_delta + args.r_on
                        noise = torch.normal(mean=1, std=torch.ones_like(r_states)*sigmaReadNoise)
                        r_noise = (r_states*noise - r_states)/min_r_delta
                        weight_noise = (param.data+1)/min_weight_delta + torch.round(r_noise)

                        idx = weight_noise > args.max_level - 1
                        weight_noise[idx] = args.max_level - 1

                        idx = weight_noise < 1
                        weight_noise[idx] = 1

                        param.data = torch.round(weight_noise)*min_weight_delta - 1
