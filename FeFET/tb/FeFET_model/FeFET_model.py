"""BTO Model for Ferroelectric Transistor Simulation"""
import math
import os
...
def runge_kutta(epsilon, e_amp, e_plot, t_plot, time_range, time_period, time_step, g_2, g_4, g_6, multi_folder,
    ...
    conductance_changes = [] # for plotting the changes in conductance over time
    polarization_last = 0  # Store the last polarization value
    i = 0
    h = time_step
    delta_p = 0
    p_next = delta_p
    ...
    pulse_counter = 0
    total_pulsed_time = 0
    pulse_now = False  # using this signal for all the square wave pulses, as opposed to just the one at the end, that way I'm not redundantly figuring out when a signal is supposed to be HIGH or LOW
    idvg_plotted = False
    last_pulse = ""
    g6_initial = g_6
    ...
    for t in time_range:
        ...
        if wave_type == "triangle":
            if time_counter >= length * 2:
                ...
                # Store polarization changes for transitions
                polarization_changes.append(p[-1])
                conductance_changes.append(p[-1] * math.pow(10, -6))  # Convert polarization to conductance
                time_counter = 0  # Reset counter for the next pulse
...
if __name__ == '__main__':
    ...
    """Voltage definitions"""
    v_start = 0
    v_end = 5  #3 for pulses, 40 for amplitude
    v_end_max = 1
    ...
    #plotting(conductance_states, vg_sweep_meta_plot, "Voltage ($V$)", "Conductance",
    #         "Capacitance", "linear", 1, 1, 1, 1, 1, False, legend_polarization_meta_plot, False, True, True, 35, -35, False, [], "", False)  



    plotting(polarization_meta_plot, vg_sweep_meta_plot, "Polarization ($μC/cm^2$)", "Voltage ($V$)",
             "Polarization Hysteresis Loops", "linear", 1, 1, 1, 1, 1, False, legend_polarization_meta_plot, False, True, True, 10, -10, False, [], "", False)  #Normally title is :"P-E Hysteresis ({}) g2_{}E6 g4_{}E8 g6_1E{}-1E{} ep_{}E21 Vprog_{}V wave_{}".format(device_type, plot_g2,plot_g4, power_min, power_max,plot_epsilon, v_end, wave_type)
    ...
    polarization_list_array = np.array(polarization_meta_plot)

    # Conductance 
    #polarization_list_array = np.array(polarization_meta_plot)
    #print((polarization_list_array.shape))

    #print(polarization_list_array) # Polarization (uC/cm^2)
    #Qg = Pg * (l * w)
    #Cg = np.diff(Qg) / np.diff(vg_sweep_meta_plot)
    #print(Cg)
    ...
    df = pd.DataFrame({"VT": vt_list_array, "g6 value": g6_list_array, "g4 value": g4_list_array, "g2 value": g2_list_array,
                        "ep value": ep_list_array, "programming Voltage": vprog_list_array, "programming pulse": pprog_list_array}) #, "Polarization": polarization_list_array})
    df.to_csv("vt list {} g2_{}E6 g4_{}E8 g6_1E{}-1E{} ep_{}E21 Vprog_{}V wave_{}.csv".format(device_type, plot_g2, plot_g4, power_min, power_max, plot_epsilon, v_end, wave_type), index=False)

    v_read = e_read * thickness_cm * math.pow(10, 3)  # Convert to voltage
    print(f"Read Voltage: {v_read} V")
    
    read_pulse = time_pulse_length  # Duration of the read pulse
    write_pulse = time_pulse_length  # Duration of the write pulse
    print(f"Read Pulse: {read_pulse} s")
    print(f"Write Pulse: {write_pulse} s")
